...
<head>
    <meta charset="UTF-8">
    <title>FinCoreHub | GL Records</title>
...
<div class="d-flex">
    ...
    <div id="content">
        <div class="top-nav"><span>GL Master / Records</span></div>
            ...
            <div class="card shadow-sm border-0 table-card">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="mb-0 text-primary fw-bold">GL Records</h5>
                    </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow">
           <div class="modal-header bg-white border-bottom">
             <h5 class="modal-title fw-bold text-primary" id="editModalLabel"><i class="bi bi-pencil-square me-2"></i>Edit GL Record</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            <form id="editGlForm">
                <div class="modal-body p-4">
                    <input type="hidden" id="editGlId">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Account Number</label>
                            <input type="text" id="editAccountNumber" class="form-control" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Console Group</label>
                            <input type="text" id="editConsoleGroup" class="form-control" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Category Group</label>
                            <input type="text" id="editCategoryGroup" class="form-control" required>
                        </div>
                    </div>
                </div>
...
<script>
    function loadRecords(pageNumber) {
        ...
        $.ajax({
            url: '${pageContext.request.contextPath}/api/gl/list',
            method: 'GET',
    ...
    function renderPagination(total, active) {
        ...
        // 2. Logic for limited numbers (Sliding window)
        const range = 2; // Pages to show before and after active page
    ...
    function openEditModal(item) {
        $('#editGlId').val(item.id);
        $('#editAccountNumber').val(item.accountNumber);
        $('#editDescription').val(item.description);
        $('#editType').val(item.type);
        $('#editBu').val(item.bu);
        $('#editConsoleGroup').val(item.consoleGroup);
        $('#editCategoryGroup').val(item.categoryGroup);
    ...
    $(document).ready(function() {
        ...
        $('#editGlForm').on('submit', function(e) {
            e.preventDefault();
            const glId = $('#editGlId').val(); 
            const updatedData = {
                accountNumber: $('#editAccountNumber').val(),
                description: $('#editDescription').val(),
                type: $('#editType').val(),
                bu: $('#editBu').val(),
                consoleGroup: $('#editConsoleGroup').val(),
                categoryGroup: $('#editCategoryGroup').val()
            };

            $.ajax({
                url: '${pageContext.request.contextPath}/api/gl/update/' + glId, 
                type: 'PUT',
                ...
                error: function(xhr) {
                    alert('Failed to update. Error status: ' + xhr.status);
                }
            });
        });
    });
